...
// ======================== VERIFF DECISION PROXY ========================
app.get("/api/veriff/decision", async (req, res) => {
  const reqId = crypto.randomUUID();
  const log = (msg, obj) => {
    console.log(`[${reqId}] ${msg}`, obj ?? "");
  };

  try {
    log("▶️ veriffDecisionProxy start");
    log("📨 method", req.method);
    log("📨 headers", req.headers);

    // Estrai session ID dai query params
    const { id } = req.query;
    ...
    if (!id || typeof id !== "string" || id.trim().length === 0) {
      ...
      return res.status(400).json({
        ...
        details: "Fornisci il parametro 'id' della sessione Veriff",
      });
    }

    log("✅ got id", id);

    // Configurazione Veriff
    const VERIFF_BASE_URL = "https://stationapi.veriff.com";
    const VERIFF_PUBLIC_KEY = process.env.VERIFF_PUBLIC_KEY;

    log(
      "🔑 using VERIFF_PUBLIC_KEY",
      VERIFF_PUBLIC_KEY ? "[set]" : "[missing]",
    );

    if (!VERIFF_PUBLIC_KEY) {
      log("❌ Missing VERIFF_PUBLIC_KEY env");
      return res.status(500).json({
        error: "Missing VERIFF_PUBLIC_KEY environment variable",
        details: "Configura VERIFF_PUBLIC_KEY nel file .env",
      });
    }

    // Costruisci URL target
    const target = `${VERIFF_BASE_URL.replace(/\/$/, "")}/v1/sessions/${encodeURIComponent(id)}/decision`;
    log("🌐 target URL", target);

    // Chiamata a Veriff API usando axios
    let upstream;
    try {
      log("🚌 sending axios request to Veriff API (GET endpoint)");
      ...
      log("❌ upstream axios request failed", String(e));
    ...
    log("⬅️ upstream status", upstream.status);
    log("⬅️ upstream headers", upstream.headers);

    const status = upstream.status;
    const contentType = upstream.headers["content-type"] ?? "application/json";
    const rawBody = upstream.data;

    log("⬅️ upstream raw body", rawBody);

    // Prova a parsare il JSON per una risposta più pulita
    let responseData;
    try {
      responseData = JSON.parse(rawBody);
    } catch (parseError) {
      // Se non è JSON valido, restituisci il raw body
      responseData = { raw: rawBody };
    }

    // Restituisci la risposta con lo stesso status code di Veriff
...
app.post("/api/veriff/decision", async (req, res) => {
  const reqId = crypto.randomUUID();
  const log = (msg, obj) => {
    console.log(`[${reqId}] ${msg}`, obj ?? "");
  };
  ...
  try {
    ...
    try {
      ...
      log("❌ invalid JSON body", String(e));
      return res.status(400).json({
        error: "Invalid JSON body",
        details: "Il body deve essere un JSON valido con campo 'id'",
      });
    }
    ...
    if (!id || typeof id !== "string" || id.trim().length === 0) {
      ...
      return res.status(400).json({
        error: "Missing required 'id'",
        details: "Fornisci il campo 'id' della sessione Veriff nel body",
      });
    }
    ...
    let upstream;
    try {
      log("🚌 sending axios request to Veriff API (POST endpoint)");
...
// ======================== VERIFF HMAC SIGNED DECISION ========================
app.get("/api/veriff/decision-signed-user", async (req, res) => {
  const reqId = crypto.randomUUID();
  const log = (msg, obj) => {
    console.log(`[${reqId}] ${msg}`, obj ?? "");
  };
  ...
  try {
    log("▶️ veriffHmacDecisionProxy start");
    log("📨 method", req.method);
    log("📨 headers", req.headers);
    ...
    // Configurazione Veriff con HMAC
    const VERIFF_BASE_URL = "https://stationapi.veriff.com";
    const VERIFF_API_KEY = "e51b1717-924b-4b5c-ab5e-69c40415ff54";
    const VERIFF_SHARED_SECRET = "a535e2da-4fdd-409d-9404-045cd653b238";

    log("🔑 using VERIFF_API_KEY", VERIFF_API_KEY ? "[set]" : "[missing]");
    log(
      "🔐 using VERIFF_SHARED_SECRET",
      VERIFF_SHARED_SECRET ? "[set]" : "[missing]",
    );

    if (!VERIFF_API_KEY || !VERIFF_SHARED_SECRET) {
      log("❌ Missing Veriff credentials");
      return res.status(500).json({
        error: "Missing Veriff credentials",
        details:
          "Configura VERIFF_API_KEY e VERIFF_SHARED_SECRET nel file .env",
      });
    }

    // Firma HMAC-SHA256 del sessionId (Veriff richiede formato hex) - GET endpoint
    const sigHex = crypto
      .createHmac("sha256", VERIFF_SHARED_SECRET)
      .update(id, "utf8")
      .digest("hex");

    log("🔐 HMAC signature generated", sigHex.substring(0, 20) + "...");

    // Costruisci URL target
    const target = `${VERIFF_BASE_URL.replace(/\/$/, "")}/v1/sessions/${encodeURIComponent(id)}/decision`;
    log("🌐 target URL", target);

    // Chiamata a Veriff API usando axios con firma HMAC
    let upstream;
    try {
      log("🚌 sending axios request to Veriff API with HMAC signature");
    ...
    res.status(status).json({
      ...
      sessionId: id,
      reqId,
      hmacSignature: sigHex.substring(0, 20) + "...", // GET endpoint
      data: responseData,
      headers: upstream.headers,
      timestamp: new Date().toISOString(),
    });
...
app.post("/api/veriff/decision-signed", async (req, res) => {
  const reqId = crypto.randomUUID();
  const log = (msg, obj) => {
    console.log(`[${reqId}] ${msg}`, obj ?? "");
  };
  ...
  try {
    ...
    // Firma HMAC-SHA256 del sessionId (Veriff richiede formato hex) - POST endpoint
    const sigHex = crypto
      .createHmac("sha256", VERIFF_SHARED_SECRET)
      .update(id, "utf8")
      .digest("hex");
    ...
    try {
      log(
        "🚌 sending axios request to Veriff API with HMAC signature (POST endpoint)",
      );
    ...
    res.status(status).json({
      ...
      sessionId: id,
      reqId,
      hmacSignature: sigHex.substring(0, 20) + "...",
      data: responseData,
      headers: upstream.headers,
      timestamp: new Date().toISOString(),
    });
