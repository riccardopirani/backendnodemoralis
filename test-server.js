...
async function testServer() {
  ...
  try {
    ...
    try {
      ...
      console.log(`📋 Contract: ${contractInfo.data.name} (${contractInfo.data.symbol})`);
    } catch (error) {
      console.log('⚠️ Contract info endpoint failed (might be expected if contract not deployed)');
    ...
    try {
      ...
      console.log('⚠️ Wallet creation failed (might be expected if Keycloak not configured)');
      console.log(`   Error: ${error.response?.data?.error || error.message}`);
    }
    console.log('');
    ...
    try {
      ...
      const balanceResponse = await axios.get(`${BASE_URL}/api/wallet/${testAddress}/balance`);
    ...
    try {
      const testAddress = '0x742d35Cc6634C0532925a3b8D4C9db96C4b4d8b6'; // Example address
      const hasJetCV = await axios.get(`${BASE_URL}/api/nft/user/${testAddress}/hasJetCV`);
      console.log('✅ NFT endpoints working');
      console.log(`🎨 Has JetCV: ${hasJetCV.data.hasJetCV}`);
    } catch (error) {
      console.log('⚠️ NFT endpoints failed (might be expected if contract not deployed)');
      console.log(`   Error: ${error.response?.data?.error || error.message}`);
    }
    console.log('');
    ...
    try {
      const testAddress = '0x742d35Cc6634C0532925a3b8D4C9db96C4b4d8b6'; // Example address
      const legacyResponse = await axios.get(`${BASE_URL}/api/user/${testAddress}/hasJetCV`);
